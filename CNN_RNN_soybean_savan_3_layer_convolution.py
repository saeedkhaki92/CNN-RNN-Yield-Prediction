...
def conv_res_part_E_with_3_layer(E_t,f,is_training,var_name):
    s0 = 1
    # First Conv Layer
    print("----Conv Layer 1 ------ Start E_t shape :", E_t.shape) # (?, 52, 1)
    X = tf.layers.conv1d(E_t, filters=8, kernel_size=13, strides=1, padding='valid',
                         kernel_initializer=tf.contrib.layers.xavier_initializer(), activation=None,
                         name='Conv00' + var_name, data_format='channels_last', reuse=tf.AUTO_REUSE)
    X = tf.nn.relu(X)
    print('conv1 out E', X)
    print('conv1 out E, X shape', X.shape) # (?, 40, 8)
    X = tf.layers.average_pooling1d(X, pool_size=2, strides=2, data_format='channels_last', name='average_pool')
    print('conv1 out E, with average pooling, X shape: ', X.shape) # (?, 20, 8)

    # Second Conv Layer
    print("------Conv Layer 2 ----- Start X shape :",X.shape) # (?, 20, 8)
    X = tf.layers.conv1d(X, filters=12, kernel_size=9, strides=1, padding='valid',
                         kernel_initializer=tf.contrib.layers.xavier_initializer(), activation=None,
                         name='Conv0' + var_name, data_format='channels_last', reuse=tf.AUTO_REUSE)
    X = tf.nn.relu(X)
    print('conv2 out E, X shape', X.shape) # (?, 12, 12)
    
    X = tf.layers.average_pooling1d(X, pool_size=2, strides=2, data_format='channels_last', name='average_pool')
    print('conv2 out E, with average pooling, X shape :', X.shape) # (?, 6, 12)


    # Third Conv Layer
    print("------Conv Layer 3 ----- Start X shape :",X.shape) # (?, 6, 12)
    X = tf.layers.conv1d(X, filters=20, kernel_size=5, strides=1, padding='valid',
                         kernel_initializer=tf.contrib.layers.xavier_initializer(), activation=None,
                         name='Conv1'+var_name, data_format='channels_last',reuse=tf.AUTO_REUSE)

    X = tf.nn.relu(X)
    print('conv3 out E, X shape', X.shape) # (?, 2, 16)
    X = tf.layers.average_pooling1d(X, pool_size=2, strides=2, data_format='channels_last', name='average_pool')
    print('conv3 out E, with average pooling, X shape :', X.shape) # (?, 1, 20)
...
def main_proccess(E_t1,E_t2,E_t3,E_t4,E_t5,E_t6,S_t1,S_t2,S_t3,S_t4,S_t5,S_t6,S_t7,S_t8,S_t9,S_t10,P_t,Ybar,S_t_extra,f,is_training,num_units,num_layers,dropout):
    ...
    # With the only three layers of convolution 
    e_out1 = conv_res_part_E_with_3_layer(E_t1, f, is_training=is_training,var_name='v1')
    e_out1 = tf.contrib.layers.flatten(e_out1)
    e_out2 = conv_res_part_E_with_3_layer(E_t2, f, is_training=is_training, var_name='v1')
    e_out2 = tf.contrib.layers.flatten(e_out2)
    e_out3 = conv_res_part_E_with_3_layer(E_t3, f, is_training=is_training, var_name='v1')
    e_out3 = tf.contrib.layers.flatten(e_out3)
    e_out4 = conv_res_part_E_with_3_layer(E_t4, f, is_training=is_training, var_name='v1')
    e_out4 = tf.contrib.layers.flatten(e_out4)
    e_out5 = conv_res_part_E_with_3_layer(E_t5, f, is_training=is_training, var_name='v1')
    e_out5 = tf.contrib.layers.flatten(e_out5)
    e_out6 = conv_res_part_E_with_3_layer(E_t6, f, is_training=is_training, var_name='v1')
    e_out6 = tf.contrib.layers.flatten(e_out6)
...
# store the csv file
    log_df.to_csv('logs/cnn_3_layes.csv')
